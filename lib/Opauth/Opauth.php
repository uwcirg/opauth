...
class Opauth {
	 ...
	 */
	public function __construct(array $config = [], bool $run = false) {
		/**
	...
	public function run(): void {
		...
		if (!empty($this->env['params']['strategy'])) {
			if (strtolower($this->env['params']['strategy']) === 'callback') {
				$this->callback();
			} elseif (isset($this->strategyMap[$this->env['params']['strategy']])) {
	...
	private function loadStrategies(): void {
		if (isset($this->env['Strategy']) && is_array($this->env['Strategy']) && count($this->env['Strategy']) > 0) {
			foreach ($this->env['Strategy'] as $key => $strategy) {
				...
				$strategyClass = $key;
				if (isset($strategy['strategy_class'])) {
				...
				$this->strategyMap[$strategy['strategy_url_name']] = [
					'name' => $key,
					'class' => $strategyClass
				];
	 ...
	 */
	public function validate(string $input, string $timestamp, string $signature, string &$reason = ''): bool {
	...
	public function callback(): void {
		...
		if (isset($response['error'])) {  // Check if it's an error callback
	 ...
	 */
	private function requireStrategy(string $strategy): string {
